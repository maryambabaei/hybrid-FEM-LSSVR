...
def create_fast_legendre_polynomials_batch(double[:, :] solutions, double[:] x_starts, double[:] x_ends, int M):
    """
    Create list of FastLegendrePolynomial objects for entire batch (optimized).
    ...
    cdef int i
    cdef list polynomials = [None] * batch_size  # Pre-allocate list - avoids repeated reallocation
    
    for i in range(batch_size):
        # Use memoryview slice directly - very fast, no copy
        polynomials[i] = FastLegendrePolynomial(solutions[i, :M], (x_starts[i], x_ends[i]))
